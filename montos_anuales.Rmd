...
# Documento que muestra un procedimiento de segmentación realizado sobre el conjunto de datos en donde se redujeron el número de variables mediante la creación de una nueva variable que es el resultado de realizar una operación entre los montos y transacciones por cada canal en donde primero se realiza la segmentación con las variables financieras escaladas y luego con el logartimo natural sin escalar de las variables de los canales.
...
```{r}
plot(1:15, colMeans(Canal_log), type = 'b', col = 'black', 
...
```{r}
dendrogram <- hclust(dist(log(grupo1_no_scaled[,2:16]+1), method = 'euclidean'), method = 'ward.D')
...
set.seed(932840)
kc1 <- kmeans(log(grupo1_no_scaled[,2:16]+1), 2 , nstart = 5, iter.max = 100)

clusplot(log(grupo1_no_scaled[,2:16]+1), kc1$cluster, color = TRUE, 
          shade = FALSE, labels = 4, lines = 1,
          col.p = kc1$cluster )
```
...
```{r}
dendrogram <- hclust(dist(log(grupo2_no_scaled[,2:16]+1), method = 'euclidean'), method = 'ward.D')
...
set.seed(93284)
kc2 <- kmeans(log(grupo2_no_scaled[,2:16]+1), 2, nstart = 5, iter.max = 100)

clusplot(log(grupo2_no_scaled[,2:16]+1), kc2$cluster, color = TRUE, 
          shade = FALSE, labels = 4, lines = 1,
          col.p = kc2$cluster )
```
...
```{r}
dendrogram <- hclust(dist(log(grupo3_no_scaled[,2:16]+1), method = 'euclidean'), method = 'ward.D')
...
set.seed(932840)
kc3 <- kmeans(log(grupo3_no_scaled[,2:16]+1), 2 , nstart = 5, iter.max = 100)

clusplot(log(grupo3_no_scaled[,2:16]+1), kc3$cluster, color = TRUE, 
          shade = FALSE, labels = 4, lines = 1,
          col.p = kc3$cluster )
```
