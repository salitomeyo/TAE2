...
Por ello, surge la necesidad de realizar un proceso de segmentación de clientes que permita a las empresas dividir a sus consumidores en categorías específicas, basadas en características que se extraen de su comportamiento como clientes y la información que pueden obtener de sus interacciones con la empresa; en este estudio en particular se está trabajando con  información de clientes corporativos, es decir, una empresa cuyos clientes también lo son.
...
El objetivo es crear grupos que tengan sentido es decir, que tengan características decifrables mediante la identificación de patrones en el uso de productos y canales del banco y su relación con los estados financieros de los diferentes clientes(empresas). Lo anterior con el fin de tomar acción sobre dichos grupos, entendiendo su comportamiento y realizando un perfil para ofrecer servicios.
...
* Realizar un correcto análisis descriptivo con el fin de transformaciones y escalamiento de variables con el fin de utilizar de manera correcta los métodos para el proceso de segmentación.

* Realizar un riguroso análisis descriptivo buscando tener las condiciones óptimas en las variables para aplicar los métodos en el proceso de segmentación.
...
kable(t(dim(df)), align = 'c', col.names = c('Filas', 'Columnas')) %>%
  kable_paper('striped', full_width = F) %>%
  row_spec(row = 1, background = '#E7E8E8')
```
...
* Hay 46 variables de las cuales, la primera es el identificador (nit) de cada empresa (cliente), las siguientes 30 reflejan el uso que cada uno de los clientes le da a los canales ofrecidos por la organización, estos se dividen en canales de entrada y de salida donde algunos miden montos de dinero y otros el número de transacciones realizadas en cada canal. Las 16 variables restantes tienen que ver con características financieras que permiten conocer algunas peculiaridades del perfil o estructura de la empresa(cliente).
...
**Gráfico de la media en logartimo natural de cada variable asociadas a los canales**
...
<center> <h2> Boxplots del logaritmo natural de las variables de los canales</h2> </center>

```{r, echo = FALSE, fig.align="center"}
par(mfrow=c(1,2))
boxplot(log(df[,1:11]+1), las=2, col="cyan4", main = "montos de entrada anual")
grid()
boxplot(log(df[,12:22]+1), las=2, col="cyan4", main = "transacciones de entrada mensual")
grid()
```
...
```{r, echo = FALSE, fig.align="center"}
par(mfrow=c(1,2))
boxplot(log(df[,23:26]+1), las=2, col="cyan4", main="montos de salida anual")
grid()
boxplot(log(df[,27:30]+1), las=2, col="cyan4", main = "transacciones de salida mensual")
grid()
```
...
<center> <h2>  Matriz de correlación asociada a las variables de los canales: </h2> </center>
...
(falta Interpretar matriz de correlación)

Las variables de los canales que presentan mayor correlación son:
...
<center> <h2> Gráficos descriptivos de cada una de las variables financieras </center> </h2>

* Gráficos de frecuencias relativas para las varibales categoricas
...
<h2> <center> Matriz de correlaciones entre las variables financieras </h2> </center>
...
<h2> <center> Agrupamiento k means con las variables de los canales (usando transformación logaritmo natural): <7h2> </center>
...
<h2> <center> Gráficos de boxplot que permitirán sacar caraterísticas de cada uno de los grupos </h2> </center>
...
<h2> <center>  Análisis del valor medio de los grupos en cada variable </h2> </center>
...
<h3> <center> media de los grupos mediante las variables financieras </h3> </center>
...
<h2> <center>Subagrupamientos mediante las variables de estados financieros usando k-means :</h2> </center>
...
```{r, echo=FALSE}
kable(aggregate(.~Grupos,data=Datos_finales_2, FUN=median), align = 'c') %>%
    kable_paper('striped', full_width = F) %>%
    scroll_box(width = "100%", height = "250px")
```
...
<h2> <center> Gráficos que permitirán sacar caraterísticas de cada uno de los grupos </h2> </center>
...
<h3> <center>  Gráfico de los grupos en las variables financieras </h3> </center>
