...
dendrogram <- hclust(dist(df_scaled, method = 'euclidean'), method = 'ward.D')
...
```{r}
km_scaled <- kmeans(df_scaled, 3, nstart = 10, iter.max = 1000)

# km_scaled$cluster

require(cluster)

clusplot(df_scaled, km_scaled$cluster, color = TRUE, 
          shade = FALSE, labels = 4, lines = 1,
