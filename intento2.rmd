...
# Documento que muestra un procedimiento de segmentación realizado primero con las variables financieras escaladas y luego con el logartimo natural sin escalar de las variables de los canales.
...
```{r}
plot(1:30, colMeans(Canal_log), type = 'b', col = 'black', 
     xaxt = "n", xlab = '', lwd = 2, ylab = 'Media')
axis(1, 1:30, names(df)[2:31], las = 2)
```

```{r}
plot(1:16, colMeans(finan_no_scaled), type = 'b', col = 'black', 
     xaxt = "n", xlab = '', lwd = 2, ylab = 'Media')
...
lines(1:16, media_df_grupfin[3, 32:47], col = "red", type = "b", lwd = 2)
...
finan1_canal_log[3:32] <- log(finan1_canal_log[3:32] + 1)
```
...
finan2_canal_log[3:32] <- log(finan2_canal_log[3:32] + 1)
```
...
finan3_canal_log[3:32] <- log(finan3_canal[3:32] + 1)
```
...
Datos_finales_log[, 4:33] <- log(Datos_finales_log[, 4:33]+1)
```
