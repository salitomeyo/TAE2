...
# Se realizan cambios en el conjunto de datos, donde en cada fila celda esta el porcetaje de uso del respectivo canal en comparación a los demás canales, teniendo en cuenta de manera independiente los montos de entrada, transacciones de entrada, montos de salida y transacciones de salida, además la segmentación se realiza primero por los canales, sin logaritmo natural y sin escalar, luego se realiza mediante las variables financieras escaladas.
...
```{r}
for (i in 1:dim(Canal_porcentaje)[2]) {
  
  col_name <- names(Canal_porcentaje)[i]
  
  hist(Canal_porcentaje[, i], xlab = col_name, main = paste('Hist de', col_name))
...
dendrogram <- hclust(dist(Canal_porcentaje, method = 'euclidean'), method = 'ward.D')
...
set.seed(93284)
Agru_canal<- kmeans(Canal_porcentaje, 2 , nstart = 5, iter.max = 100)

clusplot(Canal_porcentaje, Agru_canal$cluster, color = TRUE, 
